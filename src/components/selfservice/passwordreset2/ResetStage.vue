<template>
    <div v-if="selfServiceDetails.tag ==='end'">
        <b-alert show>
            <p>{{$t('pages.selfservice.passwordReset2.successMessage')}}</p>
        </b-alert>
    </div>
    <div v-else-if="typeof selfServiceDetails.error !== 'string'">
        <b-form @keyup.enter="save" @submit.prevent>
                <fr-policy-password-input :defaultPolicyFailures="defaultPolicyFailures" policyApi="selfservice/reset" v-model="password" name="password" :label="$t('pages.selfservice.passwordReset2.newPassword')" ></fr-policy-password-input>
            <b-button @click="save" :block="true" size="lg" variant="primary">
                {{$t("pages.selfservice.passwordReset2.changePassword")}}
            </b-button>
        </b-form>
    </div>
    <div v-else>
        <b-alert variant="danger" show>
            <b>{{$t('pages.selfservice.passwordReset2.errorMessage')}}</b>
            <p>{{selfServiceDetails.error}}</p>
        </b-alert>
        <div class="mt-2">
            <a href="#/passwordreset2" v-on:click="reloadPage">{{$t("pages.selfservice.passwordReset2.tryAgain")}}</a>
        </div>
    </div>
</template>
